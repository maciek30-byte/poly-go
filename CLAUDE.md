...
- **3-click NFR is anchored in `AppLayout`'s four nav links.** Katalog / Wiadomości / Ulubione / Profil are the contract for the PRD NFR "any of the four core capabilities within three clicks from a stable home view." `/app/dashboard` is the stable home view and is intentionally **not** in the sidebar — adding it would make five top-level items and break the contract. Adding a fifth capability requires re-validating the NFR and updating the layout (and this note).
