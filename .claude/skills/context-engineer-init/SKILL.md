...
# /context-engineer-init — Initialize /context Directory
...
- **Idempotent.** Re-running `/context-engineer-init` on a project where all six artifacts already exist is a no-op (with a status print). It must never overwrite existing content.
...
- **Not a precondition.** Other skills self-bootstrap their own files. `/context-engineer-init` is for users who like to set up the `/context` skeleton up-front.
