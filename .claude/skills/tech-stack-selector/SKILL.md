...
description: >
  ...
  scaffold. Use AFTER /prd, BEFORE /10x-bootstrapper.
...
This skill is the third link in the bootstrap chain (`/shape-notes → /prd → tech-stack-selector → /10x-bootstrapper`). Its single job: turn a written PRD into a recommended starter and a small machine-readable hand-off `/10x-bootstrapper` can read to scaffold a project.
...
**Skip when**: `context/foundation/prd.md` is absent — the skill refuses and redirects to `/shape-notes` + `/prd`. Skip also when the user is mid-implementation on an existing codebase asking about adding a library or replacing a single dependency — that is `/10x-frame` territory, not stack selection.

## Relationship to other skills

- `/shape-notes` — produces `shape-notes.md`, the precursor to PRD. Two upstream of this skill.
...
```bash
echo -n "/shape-notes" | pbcopy 2>/dev/null || echo -n "/shape-notes" | clip.exe 2>/dev/null || echo -n "/shape-notes" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/shape-notes"
```

Print verbatim (substitute the resolved path; if defaulted, this is `context/foundation/prd.md`):

```
Tech-stack-selector requires a PRD at `<prd-path>`. Run `/shape-notes` first, then re-invoke.
```
...
- question: "Are these priors correct, or do you want to correct anything before we proceed?"
  ...
  - label: "Stop — fix the PRD first"
    description: "Exit. Re-run /prd to fix priors, then re-invoke /tech-stack-selector."
  multiSelect: false
